...
export default function PortfolioCard({ item }: { item: PortfolioItem }) {
  const mockUrl = `www.${item.demoSlug}.com`

  return (
    <div className="group">
      {/* Browser window — links to demo page */}
      <Link href={`/portfolio/demo/${item.demoSlug}`} className="group/mockup block">
        <div className="rounded-xl overflow-hidden border border-white/10 shadow-2xl transition-all duration-300 group-hover/mockup:border-white/20 group-hover/mockup:shadow-[0_8px_40px_rgba(108,92,231,0.2)] group-hover/mockup:-translate-y-1.5">
          {/* Chrome bar */}
      ...
      <div className="mt-4 px-1">
        ...
        <div className="flex flex-wrap gap-2">
          <Link
            href={`/portfolio/demo/${item.demoSlug}`}
          ...
          <Link
            href={`/services/${item.niche}#pricing`}
            className="text-xs font-semibold px-4 py-2 rounded-lg border border-white/20 text-white/70 hover:text-white hover:border-white/40 transition-all"
          >
