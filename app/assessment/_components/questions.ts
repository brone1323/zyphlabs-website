...
export const QUESTIONS: Question[] = [
  ...
  // Q2 — Industry (explicit tap-through so classification is deterministic)
  {
    id: 'q2',
    kind: 'buttons',
    ...
    options: [
      ...
      { value: 'retail',                label: 'Retail (shop, cafe, restaurant, gym)', emoji: '🛍️' },
  ...
  // Q7 — Pain deep-dive A (varies by pain tag)
  {
    id: 'q7',
    kind: 'text',
  ...
  // Q8 — Industry tool stack check (generic, works for all)
  {
    id: 'q8',
    kind: 'text',
...
export function toAssessmentAnswers(raw: Record<string, any>): any {
  ...
  const company = (tradeParts[0] || '').trim() || 'Your Business'
  const trade = (tradeParts.slice(1).join(' — ') || raw.businessNameAndTrade || '').trim() || company

  return {
    reportId: `web-${Date.now()}`,
    ownerName: raw.ownerName || company + ' owner',
    ownerFirstName: (raw.ownerName || '').split(' ')[0] || 'there',
    company,
    trade,
    industry: raw.industry || 'project-based',
    customerType: raw.customerType || 'consumer',
    revenueModel: raw.revenueModel || 'per-project',
